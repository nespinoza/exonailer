...
t,f,f_err,transit_instruments,t_rv,rv,rv_err,rv_instruments = general_utils.read_data(target,mode,transit_time_def,rv_time_def)
...
if mode != 'rvs':
    t,phases,f, f_err = data_utils.pre_process(t,f,f_err,phot_detrend,\
...
if not os.path.exists('results/'+target+'_'+mode+'_'+phot_noise_model+'_'+ld_law):
    data_utils.exonailer_mcmc_fit(t, f, f_err, t_rv, rv, rv_err, \
...
if mode == 'transit':
    data_utils.plot_transit(t,f,parameters,ld_law)
elif mode == 'full':
    data_utils.plot_transit_and_rv(t,f,t_rv,rv,rv_err,parameters,ld_law,rv_jitter, \
